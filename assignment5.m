...
% Inverse Fourier Transform for each column of Ysegtrunc
ytrunc_segments = ifft(Ysegtrunc);

% Reshape the truncated signal back into a single vector
ytrunc = reshape(ytrunc_segments, M*P, 1);

% Plot the original and truncated signals over a specific time interval
% Choose an interval (for example, the first 5000 samples)
interval = 1:5000;

figure;
subplot(3, 1, 1);
plot(interval, y(interval));
...
% Plot the difference between ytrunc and y
difference = y(interval) - ytrunc(interval);
subplot(3, 1, 3);
plot(interval, difference);
title('Difference between Original and Truncated Signals');
xlabel('Sample Index');
ylabel('Amplitude');
